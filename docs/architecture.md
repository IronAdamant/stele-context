...
- **`.stele-context.toml` config**: `config.py` loads `<project_root>/.stele-context.toml` with `[stele-context]` section. Uses stdlib `tomllib` (3.11+) with minimal fallback parser for 3.10. Explicit constructor params override config file values. Supports: `storage_dir`, `chunk_size`, `max_chunk_size`, `merge_threshold`, `change_threshold`, `search_alpha`, `skip_dirs`, `respect_gitignore`, `max_history_entries`.
- **Python 3.10 floor (June 2026)**: 3.9 is EOL, so `requires-python >= 3.10`. This enabled `slots=True` on the hot `Chunk`/`IndexNode`/`Symbol` dataclasses (memory win on large indexes) and dropping the third-party `tomli` fallback. The hand-rolled minimal TOML parser stays until the floor reaches 3.11.
- **Diff-since-cache (`context_diff.py`)**: when `get_context` sees a changed file, it rebuilds the cached text from stored chunks, verifies fidelity by hashing against the stored document hash (`diff_exact`), and returns a token-bounded `difflib` unified diff with a `read_diff`/`reread_file` recommendation. Reconstruction is offset-aware: chunkers strip content but `start_pos`/`end_pos` span the raw region, so missing whitespace is re-inserted as newlines (pad to start, write content, pad to end). Several candidate rebuilds (padded, padded+`\n` for legacy rows, plain concat) are hashed; on no match the least-churn candidate is used with `diff_exact: false`. This depends on chunkers keeping offsets region-faithful — `CodeChunker` sets `end_pos` from raw region length on every branch, and `merge_similar_chunks` joins with the true offset gap, not a hardcoded `"\n\n"`. Chosen over storing full file snapshots (which would double storage) because chunks already contain the text.
- **.gitignore-aware indexing (`gitignore.py`)**: directory expansion consults the project root's `.gitignore` by default (`respect_gitignore = false` opts out). Pure-stdlib matcher supporting negation, dir-only, anchored and `**` patterns; explicitly listed files are never filtered. Scope decision: root `.gitignore` only — nested `.gitignore` files and `.git/info/exclude` are rare in practice and not worth the walk cost. The matcher is rebuilt per index/detect_changes call so edits to `.gitignore` take effect immediately. When no project root is detected (plain folder, no `.git`), `expand_paths` falls back to a matcher anchored at each indexed directory's own `.gitignore` — found in v1.4.2 verification, where a non-git folder silently indexed files its `.gitignore` excluded.
- **Bounded history auto-pruning**: `change_history` is auto-pruned to `max_history_entries` (default 1000, 0 disables) and `operation_log` to 10x that, after every `index_documents`/`detect_changes_and_update`. Rationale: telemetry logs one row per tool call and previously grew without bound. Pruning is best-effort inside the write lock and never fails the triggering operation. `get_db_health_snapshot()` reports `table_rows` + `growth_alerts` so `doctor` surfaces bloat when auto-pruning is disabled.
