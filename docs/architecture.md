...
- **Diff-since-cache (`context_diff.py`)**: when `get_context` sees a changed file, it rebuilds the cached text from stored chunks, verifies fidelity by hashing against the stored document hash (`diff_exact`), and returns a token-bounded `difflib` unified diff with a `read_diff`/`reread_file` recommendation. Chunk merging can normalize whitespace between chunks, which is why the hash check exists — an inexact rebuild still diffs correctly except for blank-line noise at old chunk boundaries. Chosen over storing full file snapshots (which would double storage) because chunks already contain the text.
