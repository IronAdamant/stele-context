...
- **Hybrid search**: HNSW finds 3x candidates; BM25 also runs `BM25Index.search()` for independent top-`2*top_k` keyword candidates, unioned with HNSW so files strong on keywords but weak on statistical signatures are not missed. `alpha * cosine + (1-alpha) * bm25`. Default `search_alpha` is 0.5. Auto-tuned alpha lowers further for natural-language queries (no code signals) and for code-like queries (identifiers, brackets). Both HNSW cosine and BM25 scores are min-max normalized within the candidate set before blending; BM25-only candidates get HNSW norm 0. Chunks with Tier 2 agent-supplied signatures get a `TIER2_BOOST = 1.3` multiplier on their vector score before blending. `has_agent_signatures(chunk_ids)` on StorageBackend provides the batch lookup.
...
- **File-path-based impact_radius**: `impact_radius(document_path=)` accepts a file path as alternative to `chunk_id`. Resolves to all chunks in the file and runs BFS from all of them. Return includes `affected_files` count alongside `affected_chunks`. Optional `compact=True` returns per-file summaries (`files`: path, chunk_count, depth_min, depth_max) instead of full `chunks` rows; `include_content=False` omits chunk text; `path_filter` substring-filters paths (e.g. exclude tests). Backward-compatible: existing `chunk_id` parameter still works.
- **detect_changes scan_new**: `detect_changes_and_update(..., scan_new=True)` with `document_paths=None` walks `project_root` with `expand_paths()` and appends unindexed files to `new` with reason `New file (scan)` (does not index them). Default `scan_new=False` preserves prior behavior (indexed paths only).
...
```bash
pip install -e ".[dev]"
pytest                    # 835 tests (835 pass, 1 skipped without mcp SDK)
mypy stele_context/
