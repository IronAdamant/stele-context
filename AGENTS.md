...
3. **`query`** — composite retrieval: semantic + symbol + text grep in one call.
...
- **Standard** (default): 42 tools, simplified surface with unified `document_lock`, `annotations`, `query`, and `batch`.
